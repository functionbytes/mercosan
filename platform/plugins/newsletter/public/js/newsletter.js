$(function () {
    ...
    var n = function (milliseconds) {
        ...
        document.cookie =
            "newsletter_popup=1; expires=" +
            expires.toUTCString() +
            "; path=/; SameSite=Lax";
        console.log(
            "Newsletter popup cookie set for:",
            cookieTime / 1000 / 60,
            "minutes",
        );
    };
    ...
    if (e.length > 0) {
            ...
            fetch(e.data("url"), {
                ...
                .then(function (response) {
                    ...
                    if (data.show_popup === false) {
                        console.log(
                            "Usuario ya suscrito o no debe mostrar popup:",
                            data.message,
                        );
                        return;
                    }
                    ...
                    setTimeout(function () {
                        ...
                        console.log(
                            "Found",
                            recaptchaElements.length,
                            "reCAPTCHA elements",
                        );

                        if (recaptchaElements.length > 1) {
                            console.log(
                                "Removing duplicate reCAPTCHA elements",
                            );
                        ...
                        if (recaptchaElements.length > 0) {
                            ...
                            if (
                                ...
                                var initRecaptcha = function () {
                                    try {
                                        var recaptchaElement =
                                            recaptchaElements[0];
                                        var sitekey =
                                            $(recaptchaElement).data("sitekey");

                                        console.log(
                                            "Rendering reCAPTCHA with sitekey:",
                                            sitekey,
                                        );

                                        var widgetId = grecaptchaObj.render(
                                            recaptchaElement,
                                            {
                                                sitekey: sitekey,
                                        ...
                                        $(recaptchaElement).data(
                                            "widget-id",
                                            widgetId,
                                        );
                                        console.log(
                                            "reCAPTCHA successfully rendered for popup with widget ID:",
                                            widgetId,
                                        );
                                    } catch (error) {
                                        console.log(
                                ...
                                if (
                                    typeof grecaptchaObj.render === "function"
                                ) {
                                    initRecaptcha();
                                } else {
                                    console.log(
                                        "Waiting for grecaptcha to load...",
                                    );
                                    setTimeout(initRecaptcha, 1000);
                                }
                            } else {
                                console.log(
                                    "grecaptcha is not available - checking if script needs to be loaded",
                                );

                                // Verificar si el script de reCAPTCHA está cargado
                                var recaptchaScript = document.querySelector(
                                    'script[src*="recaptcha"]',
                                );
                                if (!recaptchaScript) {
                                    console.log(
                                        "Loading reCAPTCHA script dynamically...",
                                    );
                                    var script =
                                    ...
                                    script.onload = function () {
                                        console.log(
                                            "reCAPTCHA script loaded, reinitializing...",
                                        );
                                        setTimeout(function () {
                                            if (
                                                typeof window.grecaptcha !==
                                                ...
                                                console.log(
                                                    "Reinitializing reCAPTCHA after script load",
                                                );
                                                var grecaptchaObj =
                                                    window.grecaptcha;
                                                var initRecaptcha =
                                                    function () {
                                                        try {
                                                            ...
                                                            console.log(
                                                                "reCAPTCHA successfully rendered after dynamic load with widget ID:",
                                                                widgetId,
                                                            );
                                                        } catch (error) {
                                                            console.log(
                                                                "reCAPTCHA render error after dynamic load:",
                                                                error,
                                                            );
                                                        }
                                                    };
                                    ...
                                    console.log(
                                        "reCAPTCHA script found but grecaptcha not ready, waiting...",
                                    );
                                    setTimeout(function () {
                                        if (
                                            typeof window.grecaptcha !==
                                            ...
                                            console.log(
                                                "reCAPTCHA ready after wait, initializing...",
                                            );
                                            var grecaptchaObj =
                                                window.grecaptcha;
                                            try {
                                                ...
                                                console.log(
                                                    "reCAPTCHA successfully rendered after wait with widget ID:",
                                                    widgetId,
                                                );
                                            } catch (error) {
                                                console.log(
                                                    "reCAPTCHA render error after wait:",
                                                    error,
                                                );
                                            }
                                        }
                    ...
                    setTimeout(function () {
                        ...
                        console.log(
                            "Popup HTML after setting:",
                            e.html().substring(0, 200),
                        );
                        ...
                        if (popupContent.length) {
                            ...
                            console.error(
                                "No .newsletter-popup-content found, cannot show modal",
                            );
                        }
                    }, t);
                })
        ...
        e.on("show.bs.modal", function () {
            ...
            dialog.css(
                "margin-top",
                Math.max(0, ($(window).height() - dialog.height()) / 2) / 2,
            );
            ...
            console.log(
                "Modal closed without action - no cookie set, will show again next visit",
            );
        });
        ...
        $(document).on("change", 'input[name="dont_show_again"]', function () {
            if ($(this).is(":checked")) {
                console.log(
                    'User checked "dont show again" - setting cookie immediately',
                );
        ...
        document.addEventListener("newsletter.subscribed", function () {
            console.log(
                "User subscribed, setting cookie immediately for 7 days",
            );
    ...
    window.recaptchaCallback = function (token) {
        console.log(
            "reCAPTCHA completed, enabling submit button, token:",
            token ? "received" : "empty",
        );
        ...
        if (recaptchaElement.length > 0) {
            try {
                ...
                if (response && response.length > 0) {
                    ...
                    console.log(
                        "reCAPTCHA not validated, keeping button disabled",
                    );
    ...
    window.newsletterPopupDebug = {
        ...
        clearPopupCookie: function () {
            ...
            console.log(
                "Newsletter popup cookie cleared. Refresh page to see popup again.",
            );
        },
        showPopup: function () {
            if (e.length > 0) {
    ...
    console.log(
        "Newsletter popup debug functions available:",
        Object.keys(window.newsletterPopupDebug),
    );
});
